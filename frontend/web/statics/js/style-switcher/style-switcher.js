...
jQuery(document).ready(function($) {
		...
		// style change
		//使用过滤选择器，标签link的属性值中含有某字符串
		var link = $("link[href*='css/skins']");
		...
		if (!($.cookie('stylesheet'))) {
			...
			link.attr('href','/statics/css/skins/' + stylesheet + '.css');
			$('.style-switcher .styleChange li[data-style="'+stylesheet+'"]').addClass("selected");
			if ($(".header-page-dark").length>0) {
				document.getElementById("logo").src="/statics/images/logo_dark_header_" + stylesheet + ".png";
			} else {
				document.getElementById("logo").src="/statics/images/logo_" + stylesheet + ".png";
			}
		}
